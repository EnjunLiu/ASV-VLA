...
#include <interfaces/msg/entity_state.hpp>
#include <interfaces/msg/entity_state_array.hpp>
#include <interfaces/msg/desired_displacement.hpp>
#include <interfaces/msg/input.hpp>
...
// find a member in a json object by its possible names
...
// read a member's value from a json object by its possible names
template<typename T>
bool read_value(
  const json & object,
  std::initializer_list<const char *> names,
...
// read a json object from a json object by its possible names
...
public:
  ...
  : Node("bridge_node")
  {
    ...
    ue_length_scale_ = declare_parameter<double>("ue_length_scale", 0.01);
    yaw_rate_scale_ = declare_parameter<double>("yaw_rate_scale", 1.0);
    yaw_rate_sign_ = declare_parameter<double>("yaw_rate_sign", 1.0); // UE5 may have a different sign convention for yaw rate.
    ...
    max_entities_ = declare_parameter<int>("max_entities", 32);
    ...
    if (port_ <= 0 || port_ > 65535) {
      throw std::runtime_error("port must be in the range of 1~65535");
    }
    ...
    entities_pub_ = create_publisher<interfaces::msg::EntityStateArray>(
      ...
      kinematic_setpoint_sub_ =
        create_subscription<interfaces::msg::DesiredDisplacement>(
            ...
            const interfaces::msg::DesiredDisplacement::SharedPtr message)
          {
            ...
            send_setpoint(*message);
          });
      controller_input_sub_ =
        create_subscription<interfaces::msg::Input>(
          "/espapp/input",
          rclcpp::QoS(10).reliable(),
          [this](const interfaces::msg::Input::SharedPtr message)
          {
            send_controller_input(*message);
          });
    }
    ...
    // Headless UE5 still needs execution_port until SceneExec shares the ObjectDeliverer socket.
...
private:
  ...
  void execution_loop()
  {
    while (running_.load()) {
      ...
      // keep the connection alive, reconnect if it is lost.
      while (running_.load()) {
        char probe;
  ...
  void process_json(const std::string & text)
  {
    try {
      ...
      // UE5 sends the floating-point game time (seconds); convert to integer microseconds to make the ROS message time consistent for the same data packet.
      ...
      // before publishing the simulation step data, set the UE5 game time as the ROS 2 time source.
      ...
      state.position_x = asv_x * ue_length_scale_;
      state.position_y = asv_y * ue_length_scale_;
      state.position_z = asv_z * ue_length_scale_;
      state.roll = roll_degrees * kPi / 180.0;
      state.pitch = pitch_degrees * kPi / 180.0;
      state.yaw = yaw_degrees * kPi / 180.0;
      state.surge_velocity = surge_velocity * ue_length_scale_;
  ...
  {
    interfaces::msg::EntityStateArray output;
    ...
    const std::string & metadata_detail)
  {
    ...
    for (std::size_t index = 0; index < entities->size(); ++index) {
      ...
      interfaces::msg::EntityState entity;
      bool visible = false;
      ...
      entity.relative_x = relative_x * ue_length_scale_;
      entity.relative_y = entity_lateral_sign_ * relative_y * ue_length_scale_;
      entity.relative_velocity_x = velocity_x * ue_length_scale_;
      entity.relative_velocity_y =
        entity_lateral_sign_ * velocity_y * ue_length_scale_;
      entity.valid = true;
      entity.confidence = 1.0F;
      entity.velocity_valid = true;
      entity.entity_embedding.fill(0.0F);
  ...
  void send_controller_input(const interfaces::msg::Input & command)
  {
    ...
    {
      std::lock_guard<std::mutex> metadata_lock(metadata_mutex_);
      have_metadata = have_last_frame_ && !last_run_id_.empty() && last_scene_seed_ > 0;
      run_id = last_run_id_;
      scene_seed = last_scene_seed_;
      frame_index = last_frame_index_;
    }
    const bool finite =
      std::isfinite(command.desired_x) && std::isfinite(command.desired_y);
    const bool executable = command.valid && finite && have_metadata;
    const double delta_x_cm = executable ?
      static_cast<double>(command.desired_x) * kinematic_position_scale_ : 0.0;
    const double delta_y_cm = executable ?
      static_cast<double>(command.desired_y) *
      kinematic_position_scale_ * kinematic_lateral_sign_ : 0.0;

    const json response = {
      {"Command_Type", "Kinematic_Setpoint"},
      {"Stamp_Us", command.stamp_us},
      {"Source_Stamp_Us", command.stamp_us},
      {"Run_ID", have_metadata ? run_id : ""},
      {"Scene_Seed", have_metadata ? scene_seed : 0},
      {"Source_Frame_Index", have_metadata ? frame_index : 0},
      {"Sequence", have_metadata ? frame_index : 0},
      {"Frame_ID", "ue_actor_local"},
      {"Source_Model_Version", "asv_vla"},
      {"Step_Dt", 0.1},
      {"Delta_X_Cm", delta_x_cm},
      {"Delta_Y_Cm", delta_y_cm},
      {"Hold_Position", !executable},
      {"Valid", executable},
      {"Reason", executable ? "OK" : "VLA_INPUT_INVALID_OR_NO_UE_FRAME"}
    };

    if (!execution_address_.empty()) {
      send_json_payload_to(response, execution_fd_, "VLA controller input");
      return;
    }
    send_json_payload(response, "VLA controller input");
  }
  ...
  // Optional secondary socket for headless SceneExec on UE5 PC.
  ...
  rclcpp::Publisher<interfaces::msg::EntityStateArray>::SharedPtr entities_pub_;
  ...
  rclcpp::Subscription<interfaces::msg::DesiredDisplacement>::SharedPtr
    kinematic_setpoint_sub_;
  rclcpp::Subscription<interfaces::msg::Input>::SharedPtr controller_input_sub_;
};
